...
if __name__ == '__main__':
	...
	tweets = pd.read_csv('dataset.csv',sep='\t')
	nonEnglish_sentences=list(tweets['tweet_text'])
	language_codes=list(tweets['lang'])
	tweets_id=list(tweets['tweet_id'])
	topics=list(tweets['topic'])
	print "ok"
	count=0
	for sentence in nonEnglish_sentences:
			to_lang="en"
			from_lang=language_codes[nonEnglish_sentences.index(sentence)]
			tweet_id=tweets_id[nonEnglish_sentences.index(sentence)]
			topic=topics[nonEnglish_sentences.index(sentence)]
		
			count+=1
			if (from_lang == "en") or (from_lang == "en-US"):
				translation = sentence
				vs = analyzer.polarity_scores(translation)
				if topic in Dictionary_score:				
					Dictionary_score[topic]+=vs['compound']
					Dictionary_count[topic]+=1
				else :
					Dictionary_score[topic]=vs['compound']
					Dictionary_count[topic]=1						
						
	for key in Dictionary_score:
		print key+"\t"+str(Dictionary_score[key]/Dictionary_count[key])
